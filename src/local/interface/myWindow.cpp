    ...
    :QMainWindow(parent),ui(new Ui::MainWindow)
{
    ...
    connect(ui->wireframe,SIGNAL(clicked()),this,SLOT(action_wireframe()));
         ...
         connect(timer, SIGNAL(timeout()), this, SLOT(action_compteur()));
         timer->start(1000);
    //connect(ui,SIGNAL(glWidget->actualise_compteur()),this,SLOT(action_compteur()));
...
void myWindow::action_compteur()
{
    ui->compteur->display(glWidget->get_compteur());
}
